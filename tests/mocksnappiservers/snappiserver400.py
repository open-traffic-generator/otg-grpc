...
def set_config():
    ...
    if test is not None and isinstance(test, bool) is False:
        ...
        return Response(status=400,
                        response=json.dumps({'errors': ['mock 400 error 1', 'mock 400 error 2']}),
                        headers={'Content-Type': 'application/json'})
...
def web_server():
    app.run(port=90, debug=True, use_reloader=False)


class SnappiServer400(object):
    def __init__(self):
        self._CONFIG = None
    ...
    def _wait_until_ready(self):
        api = snappi.api(location='http://127.0.0.1:90')
        while True:
            try:
                api.get_config()
                break
